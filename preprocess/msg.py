...
class Msg:
    ...
    def find_isolated_subbands(self, band):
        ...
        weak_subbands_compatibility = [
            self.band_compatibility(weak_subband) for weak_subband in weak_subbands
        ]
        subbands = []
        next_subband = 0
        for weak_subband, accum_comp in zip(
            weak_subbands, accumulate(weak_subbands_compatibility)
        ):
