...
static bool is_high_symmetry_line(int i, int j, const diagnose2::SpectrumData::Msg &msg) {
    std::string k1 = msg.ks[i];
    std::string k2 = msg.ks[j];

    std::vector<std::string> all_lineirreps;
    const auto &irrep_to_lineirreps = msg.k1_to_k2_to_irrep_to_lineirreps.at(k1).at(k2);
    for (const auto &[_, lineirreps] : irrep_to_lineirreps) {
        all_lineirreps.insert(all_lineirreps.end(), lineirreps.begin(), lineirreps.end());
    }

    std::sort(all_lineirreps.begin(), all_lineirreps.end());
    all_lineirreps.erase(std::unique(all_lineirreps.begin(), all_lineirreps.end()),
                         all_lineirreps.end());
...
std::vector<int> make_kpath_indices(const diagnose2::SpectrumData::Msg &msg, bool all_edges) {
    ...
    for (int i = 0; i < num_ks; ++i) {
        for (int j = 0; j < num_ks; ++j) {
            ...
            assert(is_high_symmetry_line(i, j, msg) == is_high_symmetry_line(j, i, msg));
    ...
    while (find_next_point() != -1) {
        ...
        while (!remaining_edges.at(k_idxs.back()).empty()) {
            ...
            remaining_edges[curr_k].erase(
                std::remove(
                    remaining_edges.at(curr_k).begin(), remaining_edges.at(curr_k).end(), next_k),
                remaining_edges.at(curr_k).end());
            remaining_edges[next_k].erase(
                std::remove(
                    remaining_edges.at(next_k).begin(), remaining_edges.at(next_k).end(), curr_k),
...
void complement_kpath_indices(std::vector<int> &subk_idxs,
                              const diagnose2::SpectrumData::Msg &msg) {
    for (int i = 0; i < static_cast<int>(msg.ks.size()); ++i) {
        if (std::find(subk_idxs.begin(), subk_idxs.end(), i) == subk_idxs.end()) {
