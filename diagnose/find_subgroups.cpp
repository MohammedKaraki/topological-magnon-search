...
public:
  ...
  Unitarity(const Unitarity& u)
  {
    set(u.value);
  }

  Unitarity operator*(const Unitarity& rhs) const
  {
    return Unitarity(value * rhs.value);
  }
...
public:
  ...
  PointGroupElement(const IntMatrix& m, const Unitarity u)
    : data{Data(m, u)}
  { }


  const IntMatrix& get_matrix() const
  {
    return data.first;
  }

  const Unitarity get_u() const
  {
    return data.second;
  }
...
std::string preserved_gstrs(const std::vector<std::string>& gstrs,
...
using Dirs = std::vector<std::pair<std::array<const int, 3>,
      const std::string>>;
...
std::vector<std::string> raw_input_to_vector(std::string_view gstrs)
{
  ...
  using namespace std::ranges;
  auto view = gstrs | views::split(std::string_view(";"))
...
std::vector<bool> calc_symmetry(const std::vector<PointGroupElement>& gs,
                                const auto& x)
{
...
void print_subgroups(const std::string& msg_num,
                     std::string_view raw_input)
{
  ...
  auto textify_symms_triplet =
    [&textify_symms_singlet, &dirs, &e_symms, &b_symms, &s_symms]
  ...
  auto broke_nothing = [](const std::vector<bool>& symm) {
    assert(!symm.empty());
    return std::count(symm.begin(), symm.end(), true)
      == static_cast<int>(symm.size());
  };
