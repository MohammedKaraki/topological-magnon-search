...
const auto
subsection_out_dir = "/home/mohammed/Dropbox/research/thesis/ch4-subsections";
const auto
figure_out_dir = "/home/mohammed/Dropbox/research/thesis/ch4-figures";
...
static std::string to_humanread(const std::set<int>& counts)
{
  ...
  if (counts.size() == 1) {
    ...
    result << *first << ", " << *std::next(first)
      << " or " << *std::next(std::next(first));
    return result.str();
  } else {
    auto it = first;
    auto v = *it;
...
static std::optional<std::pair<Superband, Subband>>
find_example(Superband superband,
...
std::tuple<
  ...
  >
summarize(
  const std::vector<std::pair<GapRange, SisSet>>& gap_range_sis_set_pairs,
  const int num_bands)
{
  ...
  for (const auto& [gap_range, sis_set] : gap_range_sis_set_pairs) {
    ...
    for (const auto& key_si : all_sis) {
      ...
      if (gap_end == 0) { // finished super Hamiltonian
        ...
        finalsi_to_possibcounts[key_si].insert(
          si_to_possibcounts.at(key_si).begin(),
      ...
      if (key_si == "-0") {
        for (const auto& sis : sis_set) {
          cur_possibcounts.insert(std::count_if(sis.begin(), sis.end(),
                                                [&trivial_si](const auto& si) {
                                                  return si == "-"
                                                    || si == trivial_si;
                                                }));
        }
      ...
      si_to_possibcounts[key_si] = all_sums(si_to_possibcounts.at(key_si),
                                            cur_possibcounts);
    }
  }
  ...
  finalsi_to_possibcounts["undefined (gap closed)"]
    = finalsi_to_possibcounts.at("-");
...
int main(int argc, const char** argv)
{
  ...
  {
    auto json_in = std::ifstream(fmt::format(R"({}/{}.json)",
                                             json_dir,
  ...
  const std::string main_label = fmt::format(R"({0}-{1})",
                                             json_filename_noext_nodir,
                                             data.sub_msg.number
                                            );
  ...
  const auto& positive_energy_irreps
    = data.pos_neg_magnonirreps.first;
  ...
  auto figure_filename_noext_nodir = [&json_filename_noext_nodir, &data]
    (const std::string& annotation) {
      return fmt::format("{}-{}-figure-{}",
                         json_filename_noext_nodir,
                         data.sub_msg.number,
                         annotation
                        );
    };
  auto figure_filename_noext = [&figure_filename_noext_nodir]
    (const std::string& annotation) {
      return fmt::format("{}/{}",
                         figure_out_dir,
                         figure_filename_noext_nodir(annotation)
                        );
    };
  ...
  do {
    ...
    do {
      ...
      if (gap_bracket_end >= gap_bracket_begin) {
        ...
        if (final_upper->get_trivialorgapless_count()
            >= subband.get_num_bands())
        {
          assert(final_upper->get_trivialorgapless_count()
                 == subband.get_num_bands());
  ...
  const auto trivialorgapless_counts
    = [&si_to_counts=si_to_possibcounts, &subband]() {
  ...
  const auto [low, high] = std::minmax_element(trivialorgapless_counts.begin(),
                                               trivialorgapless_counts.end());
  ...
  auto make_figure = [
    &data, low=low, high=high, &superband, &config_filename,
    &figure_filename_noext, &figure_filename_noext_nodir]
      (bool target_low) {
        ...
        subk_idxs.insert(subk_idxs.end(),
                         all_edges.begin(),
                         all_edges.end()
                        );
        {
        ...
        if (superband_subband_pair_opt) {
          const auto [superband_vis, subband_vis]
            = superband_subband_pair_opt.value();
          const auto [mode, spec] = mode_spec_pair_from_file(config_filename);
          Visualize vis(subk_idxs, superband_vis, subband_vis, data,
                        mode, spec);
          std::cerr << figure_filename_noext(low_or_high_str) + ".tex" << "\n";
          vis.dump(figure_filename_noext(low_or_high_str) + ".tex");
        }
        assert(0 == std::system(fmt::format(
              R"(set -x && pdflatex -output-directory={0} -jobname={1} )"
              R"({2}.tex 1>/dev/null && )"
  ...
  // if (!type_i_excluded)
  {
         ...
         ++it)
    {
      ...
      const bool is_first =
        it == data.subk_to_superirrep_to_subirreps.begin();
      const bool is_last =
        std::next(it) == data.subk_to_superirrep_to_subirreps.end();
...
\endlastfoot
    ...
    for (const auto& [si, possibcounts] : si_to_possibcounts) {
      ...
      doc << fmt::format(R"({0}{{{1}}})",
                         bold_or_empty,
                         to_humanread(possibcounts));
      doc << R"(\\*)" "\n";
    }
    ...
    for (auto pair_it = gap_to_possibsis.begin();
         pair_it != gap_to_possibsis.end();
         ++pair_it)
    {
      ...
      int si_counter = 1;
      for (auto it = std::next(possibsis.begin());
           it != possibsis.end();
           ++it)
      {
