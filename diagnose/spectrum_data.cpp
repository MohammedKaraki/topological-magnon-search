...
IntMatrix SpectrumData::Msg::make_br_vec(const std::vector<std::string> &br_irreps) {
...
static void from_json(const json &j, SpectrumData &data) {
    ...
    for (const auto &[subk, superirrep_to_subirreps] : data.subk_to_superirrep_to_subirreps) {
        for (const auto &[superirrep, subirreps] : superirrep_to_subirreps) {
            assert(data.super_msg.irrep_to_dim.at(superirrep) == data.sub_msg.sum_dim(subirreps));
        }
    }
...
Vector32<short> Subband::make_br(const Vector8<int> &e_idxs_beg,
...
Vector4<Vector8<int>> Subband::dimvalid_e_idxs() {
...
std::ostream &operator<<(std::ostream &out, const Supermode &supermode) {
    return out << fmt::format("({}, {})", supermode.superirrep_idx, supermode.bag_idx);
}
...
std::map<int, std::pair<bool, IntMatrix>> Subband::calc_gap_sis() const {
