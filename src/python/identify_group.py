...
def identify_group_html(gstrs):
    ...
    cache_filename = (
        "identify_group-"
        + sha256(gstrs_combined.encode('utf-8')).hexdigest()
        + ".html"
        )

    logger.info("gstrs_combined (new lines replaces with semicolons): "
                "{}\nfilename: {}".format(
        gstrs_combined.replace('\r\n', ';'),
        cache_filename))
...
def identify_group(gstrs):
    ...
    def extract_group_label_and_number():
        ...
        processed_result = raw_result \
            .replace('<font style="text-decoration: overline;">', '-') \
            .replace('</font>', '') \
            .replace('<sub>', '_{') \
            .replace('</sub>', '}') \
            .replace('<i>', '') \
        ...
        m = re.fullmatch(r"([-'0-9a-zA-Z/_{}]+) \(No\. ([0-9]+)\.([0-9]+)\)",
                         processed_result)
    ...
    def extract_transformation_matrix_entries():
        ...
        result = np.reshape(
            [float(Fraction(x)) for x in entries]
            + [0, 0, 0, 1],
            (4, 4))
