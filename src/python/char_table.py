...
from mkvec import symbol_and_vec3_from_klabel
...
class CharTable:
    ...
    def __init__(self, msg, kvector):
        ...
        self._irrep_labels, self._unitary_gs, self._char_matrix = \
            char_table_info(msg.number, self._kvector.symbol)
    ...
    def charsvec_from_g(self, g):
        ...
        almostequivs = [(idx, h) for idx, h in enumerate(self.unitary_gs)
        ...
        assert (almostequivs[1][0] - almostequivs[0][0]
                == len(self.unitary_gs) / 2)
        ...
        phase_correct_1 = np.exp(
            2.0j * np.pi * (self.kvector.coords @ dr_correct))
        phase_correct_2 = np.exp(
            2.0j * np.pi * (self.kvector.coords @ dr_incorrect))
