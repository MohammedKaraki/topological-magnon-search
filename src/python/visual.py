...
class SuperMode:
    ...
    def draw(self, x, y, color_map, output):
        ...
        sub_colors = np.array(
            [color_map[irrep] for irrep in sorted(self.sub_irreps)]
            )
    ...
    def draw_broken(self, x, y, color_map, output):
        ...
        sub_colors = np.array(
            [color_map[irrep] for irrep in self.sub_irreps]
            )
        dys = (
            .75 * (N - 1)
            * np.linspace(-.5, .5, N) 
        ...
        if N > 1:
            output.append(r"""\path [draw, dashed, draw opacity=.6, fill opacity=.1, fill={}] ({}cm, {}cm) ellipse ({}cm and {}cm);""".format(
                rgb((200,200,255)),
                x, y, .5, dys[-1]+.5
                )
                      )
        ...
        for dy, color, irrep in zip(dys, sub_colors, self.sub_irreps):
            ...
            output.append(r"""\node [dot={{.35cm}}, label={}:\huge${}$, fill={},"""
                          r"""]"""
                          r""" at ({}cm, {}cm) {{}};""".format(
                              "above" if N==1 else "below" if n==1 else "above" if n==N else "right",
                              irrep,
                              rgb(color),
                              x,
                              y + dy,
...
class Visualizer:
    ...
    def draw_super_mode(self, super_mode, x, y, broken):
        if broken:
            super_mode.draw_broken(x=x, y=y,
                                   color_map=self.color_map,
                                   output=self.nodes_code,
                                   )
        else:
            super_mode.draw(x=x, y=y,
                            color_map=self.color_map,
                            output=self.nodes_code,
                            )
    ...
    def dump(self):
        LatexBoilerplate.dump(style_code=self.style_code,
...
def test():
    ...
    draw([
SuperMode('R_{1}R_{1}', ['C_{1}^{-}C_{1}^{+}', 'C_{1}^{-}C_{1}^{+}',
                         'C_{3}^{-}C_{1}^{+}', 'C_{3}^{-}C_{1}^{+}']),
        ])

    draw([
SuperMode('S_{1}S_{1}', ['D_{1}^{+}D_{1}^{+}', 'D_{1}^{-}D_{1}^{-}', 'D_{1}^{+}D_{1}^{+}', 'D_{1}^{-}D_{1}^{-}']),
        ])
...
SuperMode('X_{1}^{-}X_{2}^{+}', ['Z_{1}^{-}Z_{1}^{+}']),
    ...
    draw([
SuperMode('U_{1}^{-}U_{1}^{-}', ['E_{1}^{-}E_{1}^{-}', 'E_{1}^{-}E_{1}^{-}']),
SuperMode('U_{2}^{-}U_{2}^{-}', ['E_{1}^{-}E_{1}^{-}', 'E_{1}^{-}E_{1}^{-}']),
        ])
