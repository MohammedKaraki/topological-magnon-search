...
IRREP_PATTERN = r'((\([A-Z]+\))?(\[overline\])?[A-Z]+_\{[0-9]+\}(\^\{[-+]\})?){1,2}\([0-9]+\)'
...
def ebrs_html(group_number):
    return cached_post(
        ...
        cache_filename=fr'mbandrep-Elementary-{group_number}.html')
...
def parse_representation(text):
    ...
    for x in text.split(r' [oplus] '):
        m = fullmatch(r'(([0-9]+) )?({})'.format(IRREP_PATTERN),
                      x)
    ...
    assert all(fullmatch(IRREP_PATTERN, x) is not None
               for x in irrep_content), irrep_content
...
def parse_tr(tr):
    ...
    row_type = None
    if (first_cell_text in ("Wyckoff pos.", "Band-rep.", "Band-type") or
            first_cell_text.startswith("Decomposable")):
...
def wyckoff_brs_html(group_number, wyckoff):
    ...
    return cached_post(
        url=r'https://www.cryst.ehu.es/cgi-bin/cryst/programs/mbandrep.pl',
        data={'super': group_number, 'wyck': 'Wyckoff', 'wyckoff':
              wyckoff_in_request},
        cache_filename=(fr'mbandrep-Wyckoff-{group_number}-'
                        + fr'{wyckoff_in_filename}.html'))
...
def kvectors_and_ebrs(group_number):
    ...
    ebrs_dict = {}
    for wyckoff, wyckoffirrep, ebr in zip(wyckoffs,
                                          wyckoffirreps,
                                          ebrs):
