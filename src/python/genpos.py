...
from utility import contents_as_str, find_unique, cleanup_genpos_html
...
def genpos_html(msg_number):
    return cached_post(
        ...
        cache_filename=fr'genpos-{msg_number}.html')
...
def fetch_gdicts(msg_number):
    ...
    for tr in table.tbody.findAll('tr', recursive=False)[2:]:
        ...
        genpos_str = cleanup_genpos_html(
            contents_as_str(
                tds[1]).split(r'<br/>')[0].replace(' ', ''))

        unitarity_sign = genpos_str[-2:]
        assert unitarity_sign in ['+1', '-1']

        mat3x4str = cleanup_genpos_html(
            contents_as_str(tds[2].table.tbody.tr.pre))

        mat3x4 = None
        if unitarity_sign == '+1':
            mat3x4 = np.reshape(
                [Fraction(x) for x in mat3x4str.split()],
                newshape=(3, 4))
        else:
            assert unitarity_sign == '-1'

        new_entry = {'counter': cleanup_genpos_html(contents_as_str(tds[0])),
                       'str': genpos_str,
                       'mat3x4': mat3x4,
                     'interpretation':
                     cleanup_genpos_html(contents_as_str(tds[3])),
                     'seitz':
                     cleanup_genpos_html(contents_as_str(tds[4])).replace(' ',
                                                                          '')}

        def sanity_check():
            test_mat = unitary_gstr_to_mat4x4(new_entry['str'])[:3, :]
            assert np.all(test_mat == new_entry['mat3x4']), (test_mat,
                                                      new_entry['mat3x4'])
        if unitarity_sign == '+1':
            sanity_check()
    ...
    return {'unitary': unitary_entries,
            'antiunitary': antiunitary_entries
            }
...
def vec4_to_str(vec4):
    ...
    joined = ''.join(fraction_to_str(frac, lett)
                     for frac, lett in zip(vec4, ['x', 'y', 'z', '']))
...
def str_to_vec4(input_str):
    ...
    for sign_g, numerator_g, letter_g, _, denominator_g \
            in findall(PART_PATTERN, s):
...
def mat3x4_to_unitary_gstr(mat):
    ...
    mat4 = np.concatenate((mat, np.array([[Fraction(0),
                                      Fraction(0),
                                      Fraction(0),
                                      Fraction(1)
                                      ]]))
...
def fetch_unitary_gs(msg_number):
    ...
    return [UnitaryGenpos.from_gstr(gdict['str'],
                                    cross_check_mat3x4=gdict['mat3x4'],
                                    seitz=gdict['seitz'])
