from magnon.diagnose2.utility.msg_info_table import (
    msg_label_from_number,
    msg_si_from_number,
...
def kvectors_and_ebrs(msg_number):
...
def _decompose_vec_into_complex_ebr_titles_helper(
    paired_vec, curr_decomp, memo, wp_and_irrep_to_paired_vec
):
...
class Msg:
    ...
    def _load_char_table(self, kvector):
        ...
        self._kvector_to_char_table[kvector] = CharTable(msg=self, kvector=kvector)
    ...
    def irreps_to_vec(self, irreps):
        ...
        b = np.array(
            [
                irreps.count(LittleIrrep(irrep_label))
    ...
    def _load_sis_and_comp_rels(self):
        ...
        semi_identity = np.diag([1] * num_bs + [0] * (sigma.shape[0] - num_bs))
        self._comp_rels = np.array(
            [
                row
                for row in (
                    (U @ semi_identity @ Uinv)
        ...
        self._si_orders = np.array([si for si in np.diag(sigma) if si > 1])
        self._si_matrix = np.array(
            [np.mod(row, si) for si, row in zip(np.diag(sigma), Uinv) if si > 1]
        )
        ...
        if len(self.si_orders) == 0:
            ...
            assert (
                ",".join([str(x) for x in self.si_orders])
                == self.symmetry_indicator[1:-1]
    ...
    def contains_unitary_g(self, g):
        ...
        for h in self.unitary_gs:
            if np.all(h.mat4x4[:3, :3] == g.mat4x4[:3, :3]) and np.all(
                np.fmod(
                    primvecsmat_inv @ (h.mat4x4[:3, 3] - g.mat4x4[:3, 3]).astype(float),
                    1.0,
                )
                == 0.0
            ):
    ...
    @property
    def label(self):
        return msg_label_from_number(self.number)

    @property
    def symmetry_indicator(self):
        return msg_si_from_number(self.number)

    @property
    def all_gstrs(self):
        return msg_gstrs_from_number(self.number)

    def comp_rels_at_ksymbol(self, ksymbol):
        assert [kvector.symbol for kvector in self.kvectors].count(ksymbol) == 1

        if ksymbol not in self._comp_rels_at_ksymbol:
            logger.info(
                "compatibility relations for {} at {} requested...".format(
                    self.number, ksymbol
                )
            )
    ...
    def __str__(self):
        return "{{{}, {}, {}}}".format(self.number, self.label, self.symmetry_indicator)

    def _irrep_tr_partners_at_kvector(self, kvector):
        def unitaryg_and_trkvector():
            def is_zero_mod_K(vec3):
                return np.allclose(
                    np.fmod(self.primvecsmat.T @ vec3.astype(float), 1.0), 0.0
                )
            ...
            unitaryg_and_trkvector_matches = [
                ...
                if is_zero_mod_K(
                    np.linalg.inv(g.mat4x4[:3, :3].T.astype(float)) @ trk.coords
                    - (-kvector.coords)
                )
            ]
        ...
        decomp_mat = mat_as_int(tr_char_mat.T.conj() @ char_mat @ normalizer)
        ...
        result = {
            original_label: assert_has_exactly_one_irrep_and_return_it(
                [
                    (tr_label, count)
        ...
        return {
            LittleIrrep(x).as_str(): LittleIrrep(y).as_str() for x, y in result.items()
        }
            ...
            for wyckoff in self.ebrs:
                for wyckoffirrep in self.ebrs[wyckoff]:
                    ...
                    assertion = (
                        Br(
                            [
                                LittleIrrep(self._tr_partners[x.as_str()])
                                for x in ebr.irreps
                            ]
                        )
                        == ebr_tr
    ...
    def make_irrep1wp_to_irreps(self):
        ...
        for wp, irrep_to_ebr in wp_to_irrep_to_ebr.items():
            for irrep in irrep_to_ebr:
                ...
                irrep1wp = "({},{})".format(irrep, wp.label)
        ...
        for wp, irrep_to_ebr in wp_to_irrep_to_ebr.items():
            for irrep in irrep_to_ebr:
                ...
                def sanity_check():
                    ...
                    ebr1_irreps_c = sorted([self.tr_partners[x] for x in ebr1_irreps])
                    ebr2_irreps_c = sorted([self.tr_partners[x] for x in ebr2_irreps])
                ...
                irrep12wp = "({},{},{})".format(irrep, irrep2, wp.label)
