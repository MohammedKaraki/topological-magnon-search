...
class SiSummary {
 private:
    struct MatrixIntCompareLess {
        bool operator()(const MatrixInt &a, const MatrixInt &b) const {
    ...
    std::map<MatrixInt, int, MatrixIntCompareLess> si_to_count;
 ...
 public:
    ...
    void increment_si(const MatrixInt &si) {
        ++si_to_count[si];
