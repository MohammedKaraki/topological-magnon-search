...
def find_unique_subtag(tag, subtag_name, attrs):
    """Returns the *unique* subtag with the given attributes. If the requested subtag is not
    unique, the function fails with an assertion.
    """
    subtags = tag.findAll(subtag_name, attrs=attrs)
    assert len(subtags) == 1, "Found {} subtags, although a unique subtag is expected!".format(
        len(subtags))
    return subtags[0]
