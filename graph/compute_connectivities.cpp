...
auto parse_input(const std::string &input) {
    ...
    auto relations =
        robin_hood::unordered_map<std::string, robin_hood::unordered_map<std::string, int>>{};
    ...
    auto irrep_label = x3::rule<class irrep_label, IrrepLabel>{} = x3::lexeme[alpha >> *alnum];
    auto point_irrep_label =
        x3::rule<class point_irrep_label, std::pair<IrrepLabel, std::optional<int>>>{} =
            irrep_label >> -('(' >> int_ >> ')');
    ...
    auto comp_rel = x3::rule<class comp_rel, std::pair<PointIrrepLabel, LineIrrepLabels>>{} =
        point_irrep_label >> "->" >> line_irrep_labels;

    auto irreps = std::vector<std::string>{};
    auto begin = input.begin();
    auto end = input.end();
    if (!x3::phrase_parse(begin, end, -(comp_rel[insert_rel]) % ';', space) || begin != end) {
...
auto extract_point_label(const std::string &point_irrep_label) {
    auto sm = std::smatch{};
    std::regex_search(point_irrep_label, sm, std::regex{R"(^[a-zA-Z]\{1,\})", std::regex::grep});
    return std::string{sm[0]};
}

auto construct_point_irrep_vecs(const auto &relations, const auto &point_irrep_counts) {
    ...
    auto result = robin_hood::unordered_map<std::string, std::vector<std::string>>{};
...
auto construct_copies_seqs(const auto &relations, const auto &point_irrep_vecs) {
    auto result =
        robin_hood::unordered_map<std::string,
                                  robin_hood::unordered_map<std::string, std::vector<int>>>{};
        ...
        for (const auto &line_irrep_label : relevant_line_irreps) {
            ...
            for (const auto &point_irrep_label : point_irrep_vec) {
                if (relations.at(point_irrep_label).contains(line_irrep_label)) {
                    copies_seq.push_back(relations.at(point_irrep_label).at(line_irrep_label));
                } else {
...
auto find_lirrep_copy_count(const auto &line_irrep_label,
    ...
    for (const auto &[point_label, _] : lirrep_labels_table) {
        if (lirrep_copies_seq_table.at(point_label).contains(line_irrep_label)) {
            const auto &count_seq = lirrep_copies_seq_table.at(point_label).at(line_irrep_label);
...
auto construct_lirrep_copy_count_table(const auto &relations,
    ...
    for (const auto &lirrep_label : lirrep_labels) {
        result[lirrep_label] =
            find_lirrep_copy_count(lirrep_label, lirrep_copy_seq_table, point_irreps_vecs);
    }
...
auto construct_column_idx_table(const auto &lirrep_count_table) {
    ...
    for (auto i = 1u; i < lirrep_and_count_arr.size(); ++i) {
        result[lirrep_and_count_arr[i].first] =
            result[lirrep_and_count_arr[i - 1].first] + lirrep_and_count_arr[i - 1].second;
    }
    return result;
}
...
auto construct_row_idx_table_and_row_names(const auto &point_irrep_vec_table) {
    ...
    for (const auto &[point_label, point_vec] : point_irrep_vec_table) {
        point_and_point_irrep_count.push_back(std::pair(point_label, point_vec.size()));
    }
...
class SymmetryData {
 public:
    SymmetryData(const std::string &input)
        : relations_and_pirrep_count{parse_input(input)},
          ...
          lirrep_copies_count_table{
              construct_lirrep_copy_count_table(relations, copies_seqs, point_irrep_vecs)},
          ...
          row_idx_table_and_row_names{construct_row_idx_table_and_row_names(point_irrep_vecs)},
 ...
 private:
    const std::pair<
        ...
        relations_and_pirrep_count;
    const robin_hood::unordered_map<std::string, robin_hood::unordered_map<std::string, int>>
        &relations;
    const robin_hood::unordered_map<std::string, int> &pirrep_count;
    const robin_hood::unordered_map<std::string, std::vector<std::string>> point_irrep_vecs;
    ...
    const std::pair<robin_hood::unordered_map<std::string, int>, std::vector<std::string>>
...
auto gen_point_matrices(const std::string &k_point,
    ...
    auto num_cols =
        std::accumulate(line_irrep_copies.begin(),
    ...
    do {
        ...
        auto curr_labels_perm = std::vector<std::string>(point_num_irreps.at(k_point));
...
void each_combination(const auto &matrix_sets, auto func) {
    using MatrixSetIterator = typename decltype(matrix_sets.begin()->second)::iterator;
...
int main() {
    ...
    const auto line_total_cols = std::accumulate(
        line_irrep_copies.begin(), line_irrep_copies.end(), 0, [](auto sum, const auto &r) {
            return sum + r.second;
        });
    ...
    const auto point_total_rows = std::accumulate(
        point_num_irreps.begin(), point_num_irreps.end(), 0, [](auto sum, const auto &r) {
            return sum + r.second;
        });
    const auto copies_seq = data.get_copies_seqs();

    auto k_point_matrix_sets = robin_hood::unordered_map<std::string, IntMatrixSet>{};
    for (const auto &point : points) {
        k_point_matrix_sets[point] = gen_point_matrices(
            point, copies_seq, point_num_irreps, line_irrep_copies, point_irreps, column_idx);
    }

    auto iters = std::vector<decltype(k_point_matrix_sets.begin()->second)::iterator>{};

    each_combination(k_point_matrix_sets, [&](const auto &labels, const auto &curs) mutable {
        auto block_matrix = IntMatrix(IntMatrix::Zero(point_total_rows, line_total_cols));

        for (auto i = 0u; i < labels.size(); ++i) {
            block_matrix.block(
                row_idxs.at(labels[i]), 0, point_num_irreps.at(labels[i]), line_total_cols) =
                *curs[i];
        }
        ...
        auto full_matrix = IntMatrix(aa_rows + bb_rows, aa_cols + bb_cols);

        full_matrix.block(0, 0, aa_rows, aa_cols).setZero();
        full_matrix.block(aa_rows, aa_cols, bb_rows, bb_cols).setZero();

        full_matrix.block(0, aa_cols, aa_rows, bb_cols) = block_matrix;

        full_matrix.block(aa_rows, 0, bb_rows, aa_cols) = block_matrix.transpose();

        laplacize(full_matrix);
        // fmt::print("{}\n\n", full_matrix);

        auto lu = Eigen::FullPivLU<Matrix>(full_matrix.cast<float>());

        auto kernel = Matrix(lu.kernel().transpose());
        kernel = kernel.block(0, 0, kernel.rows(), point_total_rows).eval();
        // fmt::print("{}\n\n", kernel);
